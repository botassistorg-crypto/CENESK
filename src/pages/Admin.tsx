...
import { 
  adminLogin, getDashboardStats, getOrders, getProducts, getBundles,
    ...
    <form onSubmit={handleSaveProduct} className="space-y-4 max-w-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input required value={productForm.name} onChange={e => setProductForm({...productForm, name: e.target.value})} className="w-full p-2.5 border border-gray-200 rounded-lg focus:outline-none focus:border-[#B8965A]" placeholder="e.g., Elegant Black Abaya" />
        </div>
        <div className="col-span-2">
        ...
        <div>
          ...
          <select value={productForm.category} onChange={e => setProductForm({...productForm, category: e.target.value})} className="w-full p-2.5 border border-gray-200 rounded-lg focus:outline-none focus:border-[#B8965A]">
      ...
      <div className="flex gap-3 pt-4">
        <button type="submit" disabled={loading} className="flex-1 bg-[#B8965A] text-white py-3 rounded-lg font-bold hover:bg-[#9A7D4B] disabled:opacity-50 transition-colors">
      ...
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#1A1A1A' }}>
        <div className="bg-[#FAF7F2] p-8 rounded-xl shadow-2xl w-full max-w-sm">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:border-[#B8965A] bg-white" />
            <button type="submit" disabled={loading} className="w-full bg-[#B8965A] text-white py-3 rounded-lg font-bold hover:bg-[#9A7D4B] transition-colors disabled:opacity-50">
              {loading ? "..." : "Enter"}
            </button>
          </form>
        </div>
    ...
    <div className="min-h-screen bg-[#FAF7F2] flex flex-col md:flex-row">
      ...
      <div className="bg-white md:w-64 border-r border-gray-200 flex-shrink-0">
        ...
        <nav className="p-4 space-y-1">
          <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'dashboard' ? 'bg-[#B8965A] text-white' : 'text-gray-600 hover:bg-gray-50'}`}>
            <LayoutDashboard className="w-5 h-5" /> Dashboard
          </button>
          <button onClick={() => setActiveTab('products')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'products' ? 'bg-[#B8965A] text-white' : 'text-gray-600 hover:bg-gray-50'}`}>
            <Package className="w-5 h-5" /> Products
          </button>
          <button onClick={() => setActiveTab('bundles')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'bundles' ? 'bg-[#B8965A] text-white' : 'text-gray-600 hover:bg-gray-50'}`}>
            <Gift className="w-5 h-5" /> Bundles
          </button>
          <button onClick={() => setActiveTab('orders')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'orders' ? 'bg-[#B8965A] text-white' : 'text-gray-600 hover:bg-gray-50'}`}>
            <ShoppingBag className="w-5 h-5" /> Orders
          </button>
          <button onClick={() => setActiveTab('customers')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'customers' ? 'bg-[#B8965A] text-white' : 'text-gray-600 hover:bg-gray-50'}`}>
            <Users className="w-5 h-5" /> Customers
          </button>
          <button onClick={() => setActiveTab('settings')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'settings' ? 'bg-[#B8965A] text-white' : 'text-gray-600 hover:bg-gray-50'}`}>
                ...
                <div>
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={openAddProduct} className="bg-[#B8965A] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#9A7D4B] flex items-center gap-2"><Plus className="w-4 h-4" /> Add Product</button>
                    </div>
                  </div>
                  {products.length === 0 ? (
                          ...
                          {products.map((product: any, index: number) => {
                            ...
                            return (
                              <tr key={product.ID || index} className="border-b border-gray-50 hover:bg-gray-50">
                                ...
                                <td className="py-3 pr-4"><div className="font-medium text-[#1A1A1A]">{product.Name}</div>{product.Featured === 'Yes' && <span className="text-xs text-[#B8965A]">⭐ Featured</span>}</td>
                                <td className="py-3 pr-4"><span className="px-2 py-1 bg-[#FAF7F2] rounded text-xs">{product.Category}</span></td>
              ...
              {isAddingBundle ? renderBundleForm() : (
                <div>
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                      <button onClick={fetchAllData} className="p-2 hover:bg-gray-100 rounded-lg" title="Refresh"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /></button>
                      <button onClick={openAddBundle} className="bg-[#B8965A] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#9A7D4B] flex items-center gap-2"><Plus className="w-4 h-4" /> Add Bundle</button>
                    </div>
                  </div>
                  {bundles.length === 0 ? (
                          ...
                          {bundles.map((bundle: any, index: number) => {
                            ...
                            return (
                              <tr key={bundle.ID || index} className="border-b border-gray-50 hover:bg-gray-50">
                                ...
                                <td className="py-3 pr-4"><div className="font-medium text-[#1A1A1A]">{bundle.Name}</div></td>
                  ...
                  <tbody>
                    {customers.map((c: any, i: number) => (
                      <tr key={i} className="border-b border-gray-50">
                        ...
                        <td className="py-3"><span className={`px-2 py-0.5 rounded-full text-xs ${c.VIPStatus === 'VIP' ? 'bg-[#B8965A]/10 text-[#B8965A]' : 'bg-gray-100 text-gray-500'}`}>{c.VIPStatus}</span></td>
                      </tr>
                    ))}
